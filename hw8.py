...
def listOfRunLengths(s):
    ...
    def lORL(s, result, curCount, curVal):
        ...
        '''if it stays the same value'''
        if s[0]==curVal:
            return lORL(s[1:],result, curCount+1, curVal)
        else:
            '''if the curval changes'''
...
def findRunBits(s):
    ...
    x=listOfRunLengths(s)
    n=math.ceil(math.log(sum(x)/len(x),2))+1
    return min(n,7)

#min(math.ceil(math.log(sum(listOfRunLengths(s))/(len(listOfRunLengths(s))),2))+1
#math.ceil(math.log(sum(listOfRunLengths(s)))/(len(listOfRunLengths(s))),2)+1
