...
def questifyAlt(str_list):
    '''Same as questify. Return a list with question mark for every item in list '''
    x=[]
    for i in str_list:
        x=x+[i+'?']
    return x
...
def catenateLoop(str_list):
    '''Same as catenate. add element together'''
    result=''
    for i in str_list:
        result=result+i
    return result
...
def wordScoreLoop(S, scorelist):
    '''Same as wordScore Result in the score of the number'''
    x=0
    for i in S:
        x=x+letterScoreLoop(i,scorelist)
    return x
...
def wordsWithScoreLambda(dct, scores):
    '''Same as wordsWithScore with lambda'''
...
def wordsWithScoreLoop(dct, scores):
    '''Same as wordsWithScore copy of the dictionary, annotated so each word is paired
    with its value. '''
    annotatedword=[]
    for i in dct:
        annotatedword= annotatedword+[[i,wordScore(i, scores)]]
